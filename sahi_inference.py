...
MODEL_WEIGHTS_PATH = os.path.join(SCRIPT_DIR, 'weights/yolov11/yolov11_best.pt')  # Update this to your model weights path

# Update this to your test images folder
BASE_DATA_DIR = sys.argv[1] if len(sys.argv) > 1 else '/users/PAS2699/brijeshnandaby/projects/cv/data/alpha-dent/AlphaDent/'
TEST_IMAGES_PATH = BASE_DATA_DIR

OUTPUT_DIR = sys.argv[2] if len(sys.argv) > 2 else os.path.join(SCRIPT_DIR, 'sahi_inference_output')
os.makedirs(OUTPUT_DIR, exist_ok=True)

SLICING_HEIGHT = int(sys.argv[3]) if len(sys.argv) > 3 else 2500  # Height of each slice
SLICING_WIDTH = int(sys.argv[4]) if len(sys.argv) > 4 else 2500   # Width of each slice
OVERLAP_RATIO = float(sys.argv[5]) if len(sys.argv) > 5 else 0.25  # 25% vertical overlap
# ---------------------------------------------------------
...
for i, image_path in enumerate(tqdm(test_images)):
    try:
        ...
        result.export_visuals(
            ...
            file_name=f"{image_id}_original3", 
            text_size=0.5, 
            rect_th=2, 
            hide_conf=True
        )
...
submission_df.to_csv(OUTPUT_DIR + '/sahi_submission.csv', index=False)
print("Saved: sahi_submission.csv")
...
high_conf_df.to_csv(OUTPUT_DIR + '/sahi_high_conf_submission.csv', index=False)
print("Saved: sahi_high_conf_submission.csv")
